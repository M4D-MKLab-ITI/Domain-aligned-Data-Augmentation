# cmodbertp.py - Prepend approach of bert. 
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, seed=12345, masking_prob=0.15, max_preds=20):
    ...
    # masked_lm_prob = 0.15
    # masked_lm_prob = 0.80
    masked_lm_prob = masking_prob
    # max_predictions_per_seq = 20
    # max_predictions_per_seq = max_seq_length//2
...
def main():
    ...
    parser.add_argument('--temp', type=float, default=1.0,
                        help="temperature")
    parser.add_argument('--model', type=str, default='bert-base-uncased', help="model name (bert-base-uncased) or \
                        or path to model directory")
    parser.add_argument('--masking_prob', type=float, default=0.15,
                        help="Masking probability of each token")
    parser.add_argument('--max_preds', type=int, default=20, help=" maximum number of masked \
...
def train_cmodbertp_and_augment(args):
    ...
    train_features = convert_examples_to_features(train_examples, label_list,
                                                  args.max_seq_length,
                                                  tokenizer, args.seed, 
                                                  args.masking_prob, args.max_preds)
