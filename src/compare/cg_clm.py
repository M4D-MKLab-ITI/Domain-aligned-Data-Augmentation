...
STOP_TOKENS = [EOS_TOKEN]#, '<'] removed because it was a bad end-condition for the generation process.
...
def main():
    ...
    parser.add_argument('--temp', type=float, default=1.0,
                        help="temperature")
    parser.add_argument('--model', type=str, default='gpt2', help="model name (gpt2) or \
                        or path to model directory")
    parser.add_argument('--eosn_index', type=int, default='64', 
                        help="when to stop generatinv examples, regardless of EOS token")

    args = parser.parse_args()
    GPT2_MODEL = args.model
...
def augment_train_data(model, tokenizer, train_examples, args):
    ...
    for ex_index, example in enumerate(train_examples):
        ...
        # amazon code - error in long texts. 
