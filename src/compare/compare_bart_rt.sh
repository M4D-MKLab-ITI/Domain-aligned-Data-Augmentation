...
ALIGNED_PATH=models/ft_bart_imdb_100_masks2s/checkpoint_best.pt
PREFIXSIZE=3
MAXEPOCH=30
TASK=rt
          ...
          do
            ...
            for MASKLEN in word span;
                ...
                fairseq-generate $DATABIN \
                        ...
                        --required-batch-size-multiple 1 --beam 10 --nbest 5 --lenpen 5 \
                        --no-repeat-ngram-size 2 \
                        --max-len-b 512 --prefix-size $PREFIXSIZE \
                        --skip-invalid-size-inputs-valid-test \
         ...
         for mr in 40;
              do
                ...
                for MASKLEN in span word;
                    ...
                    python $SRC/task/classifier.py --task $TASK --data_dir $MODELDIR --seed ${i} --cache $CACHE > $RAWDATADIR/bert_bart_l5_${MASKLEN}_mask_${MRATIO}_prefix_${PREFIXSIZE}.log --max_seq_length 512
                done
           done
    done
done
