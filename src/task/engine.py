...
class Classifier:
    ...
    def evaluate(self, set_type):
        ...
        acc_score = torch.sum(preds_all == labels_all).item() / labels_all.shape[0]
        labels_all = labels_all.cpu().numpy().reshape(-1)
        preds_all = preds_all.cpu().numpy().reshape(-1)
        report = classification_report(labels_all, preds_all, digits=4)
        gmean  = geometric_mean_score(labels_all, preds_all)
        auc = roc_auc_score(labels_all, preds_all)
        f1 = f1_score(labels_all, preds_all)
        return acc_score, report, auc, gmean, f1
