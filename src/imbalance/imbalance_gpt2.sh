...
do
    ...
    do
        ...
        do
            ...
            mkdir $GPT2DIR
            python3 $SRC/compare/cg_clm.py --model $MODEL --data_dir $RAWDATADIR --output_dir $GPT2DIR --task_name $TASK  --num_train_epochs 25 --top_p 0.9 --temp 1.0 --cache $CACHE --train_batch_size 24 --sample_num $n_samples
            ...
            python $SRC/task/classifier.py --task $TASK --data_dir $GPT2DIR --cache $CACHE > $RAWDATADIR/bert_gpt2_3.log


            #######################
            # Aligned GPT2 Classifier 
            #######################
            MODEL=models/ft_gpt2_imdb_50
            ...
            cp $RAWDATADIR/test.tsv $GPT2DIR/test.tsv
            cp $RAWDATADIR/dev.tsv $GPT2DIR/dev.tsv
            python $SRC/task/classifier.py --task $TASK --data_dir $GPT2DIR --cache $CACHE > $RAWDATADIR/bert_gpt2_turned_3.log
        done
    done 
done